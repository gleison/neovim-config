...
vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
        ...
        vim.keymap.set('n', 'rn', vim.lsp.buf.rename, { buffer = buf, desc ='[R]e[n]ame' })
        vim.keymap.set({ 'n', 'v' }, 'fs', vim.lsp.buf.format, { buffer = buf, desc = 'Format Code' })

        vim.keymap.set( { 'n', 'v' }, 'ga', vim.lsp.buf.code_action, { buffer = buf, desc ='[G]oto Code [A]ction' })
